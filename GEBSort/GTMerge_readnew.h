...
//      exit (0);
    ...
    if (st != 0)
      {
        fprintf (stderr,"failed to read more data from file %i, nextTSpoolIndex=%i\n", whatFile[nextTSpoolIndex],
                nextTSpoolIndex);
        fflush (stdout);
    ...
    if (dTS > control.dts_max && st != 2)
      {
        ...
        if (nstat->nTSjumprecover_f[whatFile[nextTSpoolIndex]] < 10)
          {
            fprintf (stderr,"timestamp jumped forward %lli for file %i;", dTS, whatFile[nextTSpoolIndex]);
            fprintf (stderr,"now=%lli; ", Event[nextTSpoolIndex].gd->timestamp);
            fprintf (stderr,"old=%lli; ", oldFileTS[nextTSpoolIndex]);
            fprintf (stderr,"nread=%i; ", control.nread);
            fprintf (stderr,"\n");
          };
        fprintf (stderr,"reject TS=%lli; file %i\n", Event[nextTSpoolIndex].gd->timestamp, whatFile[nextTSpoolIndex]);
        ...
        if (njf > control.dtsfabort)
          {
            ...
            fprintf (stderr,"abort jump forward recovery, oldFileTS %lli --> %lli; ", oldFileTS[nextTSpoolIndex],
                    Event[nextTSpoolIndex].gd->timestamp);
            fprintf (stderr,"DT=%lli\n", -lltmp);
    ...
    else if (dTS < control.dts_min && st != 2)
      {
        ...
        if (nstat->nTSjumprecover_b[whatFile[nextTSpoolIndex]] < 10)
          {
            fprintf (stderr,"TS jumped back %lli; file %i; ", dTS, whatFile[nextTSpoolIndex]);
            fprintf (stderr,"TS=%lli; ", Event[nextTSpoolIndex].gd->timestamp);
            fprintf (stderr,"old=%lli; ", oldFileTS[nextTSpoolIndex]);
            fprintf (stderr,"nread=%i, njb=%i; ", control.nread, njb);
            fprintf (stderr,"\n");
          };
        fprintf (stderr,"reject TS=%lli; file %i\n", Event[nextTSpoolIndex].gd->timestamp, whatFile[nextTSpoolIndex]);
        ...
        if (njb > control.dtsbabort)
          {
            ...
            badRead = 0;
            lltmp = oldFileTS[nextTSpoolIndex] - Event[nextTSpoolIndex].gd->timestamp;
            fprintf (stderr,"abort jump back recovery, oldFileTS %lli --> %lli; ", oldFileTS[nextTSpoolIndex],
                    Event[nextTSpoolIndex].gd->timestamp);
            fprintf (stderr,"DT=%lli\n", lltmp);
            oldFileTS[nextTSpoolIndex] = Event[nextTSpoolIndex].gd->timestamp;
          };
